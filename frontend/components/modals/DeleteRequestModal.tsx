...
export default function DeleteRequestModal({ id, name }: { id: number; name: string }) {
  const { closeModal, deleteSavedRequest } = useAppStore();
  ...
  return (
    <Modal title="Delete Request" onClose={closeModal} width="w-[420px]">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm text-[#d4d4d4]">
              Delete <span className="font-semibold text-white">"{name}"</span>?
            </p>
            <p className="text-xs text-[#6b6b6b] mt-1">This action cannot be undone.</p>
          </div>
        </div>
